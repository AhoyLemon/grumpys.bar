...
<urlset
  ...
  <?php foreach($pages->index()->visible() as $p): ?>
    ...
    if(in_array($p->uri(), $ignore)) {
    ...
    } else if ($p->expire_date()->isNotEmpty() && $p->expire_date() < date("Y-m-d")) {
      // Page is expired.
    } else if ($p->publish_date()->isNotEmpty() && $p->publish_date() > date("Y-m-d")) {
      // Page is Pending.
    } else if ($p->intendedTemplate() == "booze-type" || $p->intendedTemplate() == "menu-section") {
      // Ignore menu items as pages.
    } else if ($p->intendedTemplate() != "location" && $p->parent()->isInvisible()) {
      ...
      <url>
        ...
        <?php if ($p->teaser_pic()->isNotEmpty()) { ?>
          <image:image>
              <image:loc>
                <?= $p->url() . '/' . $p->teaser_pic(); ?>
              </image:loc>
